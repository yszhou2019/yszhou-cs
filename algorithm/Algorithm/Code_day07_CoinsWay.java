...
public class Code_01_CoinsWay {
    ...
    // sln2 发现index和aim一旦确定，返回值就确定了
    // 找出哪几个参数能够决定返回值，缓存的申请依赖于状态的决定因素
    // 就是"记忆化搜索"
	...
	// dp，也就是记忆化搜索从递归变成迭代
	// dp[idx][aim] idx表示从idx到之后的所有钱都可以考虑
	// dp[i][j]=sum(dp[i+1][j-arr[i]*0], dp[i+1][j-arr[i]*1], dp[i+1][j-arr[i]*2], dp[i+1][j-arr[i]*3] ... )
